...
`all_flash_values_path()` in `scripts/lib/all-flash.sh` picks the base from `CLOUD_PROVIDER`; both base files and the GKE variants are generated by `scripts/labs/render-all-flash-manifests.py`, so edit the renderer rather than the 32 GKE volume stanzas by hand.
