...
| 4.2 | 15–20 min | Node pool growth dominates; scale + migrations are quick |
...
The 600 GiB `mounts-budget` therefore has headroom for a node failure and for records beyond the one-page-per-sprig floor. Use this as the talking point for why the budget looks oversized for a lab with no data in it.
